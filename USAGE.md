...
## 7. Running the automated tests (optional)

The `tests/` folder contains a regression suite that loads the actual extension code into a simulated browser (via `jsdom`) and checks the detection/confidence/autofill pipeline and the popup/options UI end-to-end. It already ran clean (58/58 checks) before this was handed to you. To re-run it after making changes:
...
This doesn't touch your real browser or profile data — it's a separate, disposable simulated page.
